...
export default function DatabaseManager({
  ...
  const handleDownloadSQLite = () => {
    if (isStandalone) {
      ...
      // Production mode - Download real animalar.db binary directly
      window.open('/api/database/download', '_blank');
    }
  };

  // Handles physical .db SQLite file upload to Node.js backend
  const handleUploadSQLiteFile = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      if (isStandalone) {
        // If standalone, we accept JSON backup restore
        ...
        reader.onload = async (event) => {
          try {
            ...
            setUploadError("O ficheiro de backup selecionado é inválido.");
          } finally {
            setUploading(false);
          }
        };
        ...
        // Online Node.js mode - Upload actual binary SQLite .db file
        ...
        if (!response.ok) {
          ...
          throw new Error(errData.error || "Erro ao fazer upload do ficheiro.");
        }

        const data = await response.json();
        setUploadSuccess(true);
        
        // Let's force load from server to update the local states
      ...
      console.error(err);
      setUploadError(err.message || "Erro de conexão ao carregar a base de dados.");
    } finally {
      setUploading(false);
  ...
  const handleDownloadOfflineHTML = () => {
    ...
    // Prompts download of compiled standalone bundle from server
    window.open('/api/offline-html', '_blank');
  };
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div className="bg-slate-50/50 border border-slate-100 p-4 rounded-xl flex flex-col justify-between space-y-3">
              <div className="space-y-1">
                ...
                <p className="text-[10px] text-gray-400 leading-relaxed">
                  {isStandalone
                    ? 'Selecione o ficheiro JSON exportado anteriormente para ler e restaurar instantaneamente todos os perfis e atividades no browser.'
                    : 'Selecione e envie um ficheiro SQLite (.db) de substituição. O servidor irá recarregar e restabelecer as ligações de imediato.'}
                </p>
              </div>
              ...
              <div>
                ...
                >
                  ...
                  {uploading ? 'A carregar...' : `Importar ${isStandalone ? 'JSON' : 'SQLite (.db)'}`}
                </button>
              </div>
            </div>
